...
Similar constraints apply to the covariance matrix, $\bbSigma$.
...
Note that the standardizing transformation gives us a way to simulate values from a general $n$-dimensional multivariate Gaussian, $\mathcal{X}\sim\mcN(\bbmu,\bbSigma)$ as
$$
\bbx=\bbmu+\mathbf{L}\mathbf{z}
$$ {#eq-simulatemvn}
where $\mathbf{z}$ is simulated from the $n$-dimensional *standard multivariate Gaussian*, $\mathcal{Z}\sim\mcN(\mathbf{0},\bbI)$, which is $n$ independent univariate standard normal distributions.

### Linear functions of a multivariate Gaussian

In general, if $\mathcal{X}$ is an $n$-dimensional random variable with mean $\bbmu$ and covariance matrix $\bbSigma$, and $\mathbf{A}$ is a matrix with $n$ columns then the mean and variance of $\mathcal{U}=\mathbf{A}\mathcal{X}$ are given by
$$
\require{unicode}
...
&=𝔼\left[\left(\mathbf{A}\mathcal{X}-\mathbf{A}\bbmu\right)\left(\mathbf{A}\mathcal{X}-\mathbf{A}\bbmu\right)'\right]\\
&=𝔼\left[\mathbf{A}\left(\mathcal{X}-\bbmu\right)\left(\mathcal{X}-\bbmu\right)'\mathbf{A}'\right]\\
&=\mathbf{A}\,𝔼\left[\left(\mathcal{X}-\bbmu\right)\left(\mathcal{X}-\bbmu\right)'\right]\mathbf{A}'\\
&=\mathbf{A}\text{Var}(\mathcal{X})\mathbf{A}'\\
&=\mathbf{A}\bbSigma\mathbf{A}'
\end{aligned}
$$ {#eq-mvvarlin}

A linear function, $\mathcal{U}=\mathbf{A}\mathcal{X}$, of a multivariate Gaussian distribution, $\mathcal{X}\sim\mcN(\bbmu,\bbSigma)$, is also Gaussian and these relationships imply that
$$
\mathcal{U}\sim\mcN(\mathbf{A}\bbmu, \mathbf{A}\bbSigma\mathbf{A}')
$$ {#eq-mvnlinfunc}
...
@eq-linmoddensity describes the density of the random variable, $\mathcal{Y}$, representing the observations, given the values of the parameters, $\bbbeta$ and $\sigma^2$.
For parameter estimation we use the *likelihood function*, which is the same expression as @eq-linmoddensity but regarded as function of the parameters, $\bbbeta$ and $\sigma^2$, with the observed response, $\bby$, fixed.
$$
...
-1
\end{bmatrix}\right\|^2\\
&=\left\|\bbR_{XX}\bbbeta-\mathbf{r}_{Xy}\right\|^2
...
$$
\begin{aligned}
(\mathcal{Y}|\mathcal{B}=\mathbf{b})&\sim\mcN(\bbX\bbbeta+\mathbf{Z}\mathbf{b},\sigma^2\bbI)\\
\mathcal{B}&\sim\mcN(\mathbf{0},\bbSigma_\theta) .
\end{aligned}
$$

The $q\times q$, symmetric, variance-covariance matrix, $\mathrm{Var}(\mathcal{B})=\bbSigma_\theta$, depends on the *variance-component parameter vector*, $\bbtheta$, through a lower triangular *relative covariance factor*, $\Lambda_\theta$ as
$$
...
Many computational formulas for linear mixed models are written in terms of the *precision matrix*, $\bbSigma_\theta^{-1}$.
...
However, we will blur that distinction and continue to write $\text{Var}(\mathcal{B})=\bbSigma_\theta$.
...
However, $\Lambda_\theta$, like $\bbSigma_\theta$, can be singular.

The computational methods in the *MixedModels* package are based on $\Lambda_\theta$ and do not require evaluation of $\bbSigma_\theta$.
In fact, $\bbSigma_\theta$ is explicitly evaluated only at the converged parameter estimates.

The spherical random effects, $\mathcal{U}\sim\mcN(\mathbf{0},\sigma^2\bbI_q)$, determine $\mathcal{B}$ as
$$
...
𝔼\left[\bbLambda_\theta\mathcal{U}\right]=
\bbLambda_\theta\,𝔼\left[\mathcal{U}\right]=
\bbLambda_\theta\mathbf{0}=\mathbf{0}
$$
and covariance matrix
$$
\text{Var}(\mathcal{B})=
\bbLambda_\theta\text{Var}(\mathcal{U})\bbLambda\theta'=
\sigma^2\bbLambda_\theta\bbLambda_\theta'=\bbSigma_\theta
$$

Just as we concentrate on how $\bbtheta$ determines $\Lambda_\theta$, not $\bbSigma_\theta$, we will concentrate on properties of $\mathcal{U}$ rather than $\mathcal{B}$.
In particular, we now define the model according to the distributions
$$
\begin{aligned}
(\mathcal{Y}|\mathcal{U}=\mathbf{u})&\sim\mcN(\mathbf{Z}\Lambda_\theta\mathbf{u}+\bbX\beta,\sigma^2\bbI_n)\\
\mathcal{U}&\sim\mcN(\mathbf{0},\sigma^2\bbI_q) .
\end{aligned}
$$ {#eq-condygivenu}
...
That is
$$
f_{\mathcal{Y},\mathcal{U}}(\bby,\mathbf{u})=
...
To evaluate the likelihood for the parameters, $\bbtheta$, $\bbbeta$, and $\sigma^2$, given the observed response, $\bby$, we must evaluate the marginal distribution of $\mathcal{Y}$, which is the integral of $f_{\mathcal{Y},\mathcal{U}}(\bby,\mathbf{u})$ with respect to $\mathbf{u}$.

This is much simpler if we first rewrite the *penalized sum of squared residuals*, $\left\|\bby-\bbX\bbbeta
-\mathbf{Z}\bbLambda_\theta\mathbf{u}\right\|^2+
...
$$
L(\bbtheta,\bbbeta,\sigma|\bby) = \int_\mathbf{u} f_{\mathcal{Y},\mathcal{U}}(\bby,\mathbf{u})\, d\mathbf{u}
...
$$
\mathcal{Y}\sim\mcN(\bbX\bbbeta,\sigma^2\bbI_n),
$$ {#eq-20}
