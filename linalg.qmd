...
According to @eq-relcovfac, $\boldsymbol{\Sigma}$ depends on both $\sigma$ and $\theta$, and we should write it as $\boldsymbol{\Sigma}_{\sigma,\theta}$.
...
However, the only way for its estimate, $\widehat{\sigma}$, to be zero is for the fitted values from the fixed-effects only, $\mathbf{X}\widehat{\boldsymbol{\beta}}$, to be exactly equal to the observed data.
