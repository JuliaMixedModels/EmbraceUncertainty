...
using LinearAlgebra
using MixedModels
# XXX can remove when associated PR is merged for raneftables
using TypedTables
```
...
Often this optimization requires hundreds or thousands of evaluations of the objective at different values of the parameters and the these evaluations dominate the overall estimation time.
...
This property is useful for avoiding allocation of storage in, for example, evaluation of objective function during optimization.
...
```{julia}
sleepstudy = Table(MixedModels.dataset(:sleepstudy))
...
-\mathbf{Z}\bbLambda_\theta\mathbf{u}\right\|^2+
\left\|\mathbf{u}\right\|^2$, from @eq-yujointdensity,  a quadratic form in $\mathbf{u}$, to isolate the dependence on $\mathbf{u}$
