...
The $q\times q$, symmetric, variance-covariance matrix, $\mathrm{Var}(\mcB)=\bbSigma_\theta$, depends on the *variance-component parameter vector*, $\bbtheta$, through a lower triangular *relative covariance factor*, $\Lambda_\theta$ as
...
Although it may seem more intuitive to write $\mcU$ as a linear transformation of $\mcB$, we cannot do that when $\Lambda_\theta$ is singular, which is why @eq-sphericalre is in the form shown.
...
Just as we concentrate on how $\bbtheta$ determines $\Lambda_\theta$, not $\bbSigma_\theta$, we will concentrate on properties of $\mcU$ rather than $\mcB$.
...
The joint density for $\mcY$ and $\mcU$ is the product of densities of the two distributions shown in @eq-condygivenu.
That is
