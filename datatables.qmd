...
The data sets in the `MixedModels` package and the auxillary data sets used in this book are stored in the Arrow file format and retrieved as `Arrow.Table`s.
...
## Tables.jl provides an "interface for tables"
...
In *relational database management systems* (RDBMS), such as [PostgreSQL](https://postgresql.org), [SQLite](https://sqlite.org), and a multitude of commerical systems, a data table, called a *relation*, is typically stored row-wise.
...
```{julia}
isD49dist(row) = row.dist == "D49"    # a simple 'one-liner' function definition
