...
We retain the concept of the linear predictor, $\bf X\bf\beta+\bf Z\bf b$, depending on the fixed-effects parameters, $\bf\beta$, and the random effects, $\bf b$, with the corresponding model matrices, $\bf X$ and $\bf Z$, determining the conditional mean, $\bf\mu$, of the response, given the random effects, but the relationship is more general than that for the linear mixed model.
The linear predictor, which we shall write as $\bf\eta$, determines $\bf\mu$ according to a *link function*, $g$.
For historical reasons it is the function taking an element of $\bf\mu$ to the corresponding element of $\bf\eta$ that is called the link.
The transformation in the opposite direction, from $\bf\eta$ to $\bf\mu$, is called the *inverse link*.
...
Models incorporating coefficients in a linear predictor but allowing for more general forms of the distribution of the response are called *generalized linear models*.
...
The data are a subsample of responses from 1934 women grouped in 60 districts and are available as the data set in the `MixedModels` package.
```{julia}
...
Note that the variable is centered about a particular age so some values are negative.
...
Although the coefficients labeled `livch: 1`, `livch: 2`, and `livch: 3+` are all large relative to their standard errors, they are quite close to each other.
...
```{julia}
com04 = let
  form = @formula(use ~ 1 + ch + urban + age + age^2 + (1|dist))
...
```{julia}
com05 = let
  form = @formula(use ~ 1 + ch + urban + age + age^2 + (1|dist&urban))
  contrasts = Dict(:urban => HelmertCoding(), :dist => Grouping())
  fit(MixedModel, form, contra, Bernoulli(); contrasts, progress)
end
```
