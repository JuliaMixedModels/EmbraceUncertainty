...
That is, `L` requires roughly 6 times the amount of storage as does `A`, and that difference is entirely due to the different structure of the [2,2] block.
...
@fig-memoryfootprint shows that when all the movies are included in the data to which the model is fit (i.e. `mc == 1`) the total memory footprint is over 25 GiB, and nearly 90% of that memory is that required for the `[2,2]` block of `L`.
Even when requiring a minimum of 50 ratings per movie, the `[2,2]` block of `L` is still close to a third of the memory footprint.
