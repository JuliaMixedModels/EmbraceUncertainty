...
```{julia}
ratings = EmbraceUncertainty.dataset("ratings")
```

Convert this table to a `DataFrame` and drop the `timestamp` column, which we won't be using.
...
let
  ...
  for (i, n) in zip(round.(Int, Float32(2) .* ratings.rating), ratings.nrtngs)
    if n > 4
      fiveplus[i] += 1
    elseif isone(n)
      onlyone[i] += 1
