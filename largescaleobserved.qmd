...
We analyze the *MovieLens 32M Dataset* of roughly 32 million ratings of movies by over 200,000 users of about 84,000 movies.

These data are redistributed as Arrow files by the [MixedModelsDatasets](https://github.com/JuliaMixedModels/MixedModelsDatasets.jl) package.
The first time that `dataset(:ml32_ratings)` is executed, a 151 Mb file is downloaded and cached; subsequent calls read it from the cache.
...
Titles and genres for the movies are in a companion dataset, `ml32_movies`, which is keyed by the same `movieId`.
Neither table records how many ratings each movie received, so we count them, sort from the most-rated movie to the least, and attach the titles.
Note that this counts only movies that were actually rated: `ml32_movies` describes a few thousand more movies than appear in `ml32_ratings`.

```{julia}
movies = leftjoin!(
  sort!(
    combine(
      groupby(DataFrame((; ratings.movieId)), :movieId),
      nrow => :nrtngs,
    ),
    :nrtngs;
    rev=true,
  ),
  DataFrame(dataset(:ml32_movies));
  on=:movieId,
)
disallowmissing!(movies; error=false)
```

The `userId` and `movieId` columns are stored as integers.
We convert them to zero-padded strings tagged with `U` or `M`, both because these are identifiers rather than numbers and because padding them to a constant width makes them sort in the same order as the original integers.
The result is a `PooledArray`, which stores each distinct level once, so this also uses less memory than the original integer columns.

At the same time we drop the `timestamp` column, which we won't be using.
(For small data sets dropping such columns is not important but with 32 million ratings it does help to drop unnecessary columns to save some memory space.)

```{julia}
ratings = Table(;
  userId=tagpad(ratings.userId, "U"),
  movieId=tagpad(ratings.movieId, "M"),
  rating=Float32.(ratings.rating),
)
```

Because `movies` was built from the ratings, its `movieId` column contains exactly the movie identifiers that occur in `ratings`, so tagging it separately produces the same labels.

```{julia}
movies = Table(
  transform!(movies, :movieId => tagpad("M") => :movieId),
)
```
...
The `nrtngs` column gives the number of ratings for each movie, which varies from 1 to over 100,000.
...
In this collection of about 32 million ratings, over 20% of the movies are rated only once and over half of the movies were rated 6 or fewer times.
...
For example, the minimum number or movies rated is 20 (due to the inclusion constraint); the median number of movies rated is around 70; but the maximum is over 33,000 (which is a lot of movies - over 30 years of 3 movies per day every day - if this user actually watched all of them).
...
In this table, `mc` is the "movie cutoff" (i.e. the threshold on the number of ratings per movie); `uc` is the user cutoff (threshold on the number of ratings per user); `nratings`, `nusers` and `nmvie` are the number of ratings, users and movies in the resulting trimmed data set; `modelsz` is the size (in GiB) of the model fit; `L22sz` is the size of the [2,2] block of the `L` matrix in that model; `fittime` is the time (in seconds) required to fit the model; `nv` is the number of function evaluations until convergence; and `evtime` is the time (s) per function evaluation.

The "[2,2] block of the `L` matrix" is described in @sec-lrgobsmemprint.

### Dimensions of the model versus cut-off values

As shown if @fig-nratingsbycutoff, the number of ratings varies from a little over 27 million to 32 million, and is mostly associated with the user cutoff.
...
This model can be restored in a couple of minutes from the saved `optsum` values, as opposed to the twenty minutes needed to perform the fit.
The larger models in the table are restored the same way, but they are much slower to restore --- and far slower still to fit, with the `mc = 1` fits taking upwards of fourteen hours.
...
function ratingsoptsum(
  ...
  # these must match scripts/movielens_sizespeed.jl, which generated the
  # saved `optsum` values for the other cutoff pairs
  model.optsum.initial .= 0.5
  model.optsum.ftol_rel = 1.0e-10
...
Each evaluation of the objective, which requires setting the value of the parameter $\bbtheta$ in the numerical representation of the model, updating the blocked Cholesky factor, $\mathbf{L}$, and evaluating the scalar objective value from this factor, takes about 45 seconds on a server node and probably longer on a laptop.
...
That is, `L` requires roughly 4 times the amount of storage as does `A`, and that difference is entirely due to the different structure of the [2,2] block.
...
The matrix is about 98% zeros or, equivalently, a little under 2% nonzeros,
...
@fig-memoryfootprint shows that when all the movies are included in the data to which the model is fit (i.e. `mc == 1`) the total memory footprint is nearly 30 GiB, and over 90% of that memory is that required for the `[2,2]` block of `L`.
Even when requiring a minimum of 50 ratings per movie, the `[2,2]` block of `L` is still more than a quarter of the memory footprint.

The amount of storage required for the `[2,2]` block can be nearly cut in half by taking advantage of the fact that it is a triangular matrix.
`MixedModels.jl` since v5.8.0 does this automatically using the [rectangular full packed (RFP) format](https://netlib.org/lapack/lawnspdf/lawn199.pdf) for diagonal blocks whose dimension exceeds `RFPthreshold` (default 1000).
The `L22sz` column of `sizespeed` records the size of the RFP representation.
The block is square with one row and column per movie, so the storage that the older, dense representation would have required can be computed directly from `nmvie` and compared against it.
...
select(
  transform(
    sizespeed,
    :nmvie => ByRow(n -> Int64(n)^2 * 8 / 2^30) => :L22dense,
  ),
  :mc,
  :uc,
  :nmvie,
...
For a single model the saving can be seen directly, by constructing `mvm50u80` again with RFP storage disabled.

```{julia}
#| code-fold: true
mvm50u80_dense = ratingsoptsum(50, 80; RFPthreshold=typemax(Int))
NamedTuple{(:RFP, :dense)}(
  Base.format_bytes.(Base.summarysize.(getproperty.((mvm50u80, mvm50u80_dense), :L))))
```
...
The reason for considering `evtime` in addition to `fittime` and `nv` is because the `evtime` for one model, relative to other models, is reasonably stable across computers whereas `nv`, and hence, `fittime`, can be affected by seemingly trivial variations in function values resulting from different implementations of low-level calculations, such as the BLAS (Basic Linear Algebra Subroutines).

To reiterate, the number of iterations to convergence, `nv`, is highly variable.
Most of these models required around 30 evaluations, but the number ranges from 26 to 38.
Thus we can't expect to reproduce `nv` exactly when fitting the same model on different computers or with slightly different versions of software but the pattern in `evtime` with respect to `uc` and `mc` can be expected to reproducible.
