...
```{julia}
ratings = Table(getproperties(ratings, (:userId, :movieId, :rating)))
```
...
```{julia}
users = Table(combine(groupby(DataFrame(ratings), :userId),
                      nrow => :urtngs,
                      :rating => mean => :umnrtng))
```
...
let
    ...
    ecdfplot(f[1, 1],
             nrtngs;
             npoints=last(nrtngs),
             axis=(xlabel="Number of ratings per movie (logarithmic scale)",
                   xminorgridvisible,
                   xminorticks,
                   xminorticksvisible,
                   xscale,
    ...
    ecdfplot(f[1, 2],
             urtngs;
             npoints=last(urtngs),
             axis=(xlabel="Number of ratings per user (logarithmic scale)",
                   xminorgridvisible,
                   xminorticks,
                   xminorticksvisible,
                   xscale,
...
```{julia}
ratings = Table(disallowmissing!(leftjoin!(leftjoin!(DataFrame(ratings),
                                                     select!(DataFrame(movies), :movieId,
                                                             :nrtngs);
                                                     on=:movieId,),
...
```{julia}
#| code-fold: true
function ratingsoptsum(mcutoff::Integer,
                       ucutoff::Integer;
                       data=Table(ratings),
                       form=@formula(rating ~ 1 + (1 | userId) + (1 | movieId)),
                       contrasts=contrasts,)
    optsumfnm = optsumdir("mvm$(lpad(mcutoff, 2, '0'))u$(lpad(ucutoff, 2, '0')).json")
    model = LinearMixedModel(form,
                             filter(r -> (r.nrtngs ≥ mcutoff) & (r.urtngs ≥ ucutoff),
                                    data);
                             contrasts,)
...
```{julia}
#| code-fold: true
NamedTuple{(:A, :L)}(Base.summarysize.(getproperty.(Ref(mvm50u80), (:A, :L))) ./ 2^30)
```
