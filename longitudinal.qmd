...
```{julia}
#| code-fold: true
#| fig-cap: "Length of ramus bone versus age for a sample of 20 boys.  The panels are ordered rowwise, starting at the top left, by increasing initial bone length."
#| label: fig-eglayout
...
egm01 = let
    ...
    fit(MixedModel, form, egdf; contrasts, progress)
end
DisplayAs.Text(egm01)
```
...
egm02 = let
    form = @formula(resp ~ 1 + time + (1 + time | Subj))
    contrasts = Dict(:time => Center(8), :Subj => Grouping())
    fit(MixedModel, form, egdf; contrasts, progress)
end
DisplayAs.Text(egm02)
```
...
```{julia}
egm04 = let
    form = @formula(resp ~ 1 + ctime + ctime^2 + (1 + ctime + ctime^2 | Subj))
    dat = transform(egdf, :time => (x -> x .- 8.75) => :ctime)
    contrasts = Dict(:Subj => Grouping())
    fit(MixedModel, form, dat; contrasts, progress)
end
DisplayAs.Text(egm04)
```
...
```{julia}
nrow(unique(select(bxdf, :Group, :Subj))) == length(unique(bxdf.Subj))
```
...
```{julia}
combine(groupby(unique(select(bxdf, :Group, :Subj)), :Group), nrow => :n)
```
...
```{julia}
#| code-fold: true
#| fig-cap: "Weight (g) of rats in the control group versus time in trial (wk). Panels are ordered by increasing initial weigth."
...
```{julia}
#| code-fold: true
#| fig-cap: "Weight (g) of rats in the Thioracil group versus time in trial (wk). Panels are ordered by increasing initial weigth."
...
```{julia}
#| code-fold: true
#| fig-cap: "Weight (g) of rats in the Thyroxingroup versus time in trial (wk). Panels are ordered by increasing initial weigth."
...
bxm01 = let
    ...
    contrasts = Dict(:Subj => Grouping())
    fit(MixedModel, form, bxdf; contrasts, progress)
end
DisplayAs.Text(bxm01)
```
...
```{julia}
bxm03samp = parametricbootstrap(Xoshiro(8642468), 10_000, bxm03; hide_progress=true)
