...
Data from a dental study measuring the lengths of the ramus of the mandible (mm) in 20 boys at 8, 8.5, 9, and 9.5 years of age were reported in @elstongrizzle1962 and in @davis2002.
...
```{julia}
#| code-fold: true
#| fig-cap: Length of the ramus of the mandible versus age for a sample of 20 boys.
#| label: fig-egspaghetti
draw(
  data(egdf) *
  mapping(
    :time => "Age (yr)",
    :resp => "Length of the ramus of the mandible (mm)",
    color=:Subj,
  ) *
...
```{julia}
#| code-fold: true
#| fig-cap: Length of the ramus of the mandible versus age for a sample of 20 boys.  The panels are ordered rowwise, starting at the bottom left, by increasing bone length at age 8.
#| label: fig-eglayout
...
print(
  lattice::xyplot(
    ...
    xlab="Age (yr)",
    ylab="Length of the ramus of the mandible (mm)",
    ),
  )
""";
```
...
```{julia}
#| code-fold: true
#| fig-cap: Weight (g) of rats in the Thyroxin group versus time in trial (wk). Panels are ordered by increasing initial weigth.
...
bxm03stbl = bxm03samp.tbl
```

```{julia}
Table(shortestcovint(bxm03samp))
```
...
draw(
  data(bxm03stbl) *
  mapping(
    [:ρ1, :ρ2, :ρ3] .=> "Bootstrap samples of correlations of random effects",
  ) *
  mapping(color=dims(1) => renamer(["ρ₁", "ρ₂", "ρ₃"])) *
...
draw(
  data(bxm03stbl) *
  mapping(
    [:ρ1, :ρ2, :ρ3] .=> (x -> atanh(clamp(x, -0.99999, 0.99999))).=> "Fisher's z transformation of correlation bootstrap estimates",
  ) *
  mapping(color=dims(1) => renamer(["ρ₁", "ρ₂", "ρ₃"])) *
  AlgebraOfGraphics.density();
  figure=(; resolution=(800, 450)),
