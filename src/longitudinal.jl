...
function eglayout!(egdf)
    if "orderedsubj" ∉ names(egdf)
        egdf.orderedsubj = CategoricalArray(
            egdf.Subj; levels=@sort(@subset(egdf, :time == 8.0), :resp).Subj
        )
    end
    egplt = data(egdf) * mapping(:time => "Age (yr)", :resp => "Ramus bone length (mm)")
...
function bxctrllayout(bxdf, bxaxes)
    ...
    df.orderedsubj = CategoricalArray(
        df.Subj; levels=sort(@subset(df, iszero(:time)), :resp).Subj, ordered=true
    )
    draw(
...
function fitegm02(egdf)
    return fit(
        ...
        @formula(resp ~ 1 + time + (1 + time | Subj)),
        egdf;
        contrasts=Dict(:Subj => Grouping(), :time => Center(8)),
        progress=false,
        thin=1,
    )
end
...
function fitegm03(egdf)
    return fit(
        ...
        @formula(resp ~ 1 + time + (1 + time | Subj)),
        egdf;
        contrasts=Dict(:Subj => Grouping(), :time => Center()),
        progress=false,
        thin=1,
    )
end
...
function bxm03plane(bxm03)
    ...
    ax2 = Axis3(
        f[1, 2]; aspect, xlabel, ylabel, zlabel, perspectiveness, elevation, azimuth
    )
...
function bxm03rhodensatanh(bxm03pars)
    draw(
        data(
            @transform(
                @subset(bxm03pars, :type == "ρ"), :z = atanh(clamp(:value, -0.9999, 0.9999))
            )
        ) *
...
function bxmodfitted(bxm03beta, bxm04beta, bxm05beta)
    ...
    draw(
        data(
            append!(
                append!(
                    DataFrame(;
                        times=tmat[:, 2],
                        wt=tmat * Float32.(bxm03beta),
                        Group=grp,
                        model="bxm03",
                    ),
                    DataFrame(;
                        times=tmat[:, 2],
                        wt=tmat * Float32.(bxm04beta),
                        Group=grp,
                        model="bxm04",
                    ),
                ),
        ...
        mapping(
            :times => "Time in trial (wk)", :wt => "Weight (gm)"; color=:Group, col=:model
        ) *
        visual(Lines);
