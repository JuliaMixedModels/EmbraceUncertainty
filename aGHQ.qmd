...
As in the case of the linear mixed model, the *covariance factor*, $\bbLambda_{\bbtheta}$, is sparse and patterned.
...
#| output: false
#| label: packagesA03
using AlgebraOfGraphics: data, draw, mapping
using CairoMakie
CairoMakie.activate!(; type="svg")
using Chairmarks: @b # like @benchmark but better "back support"
using EmbraceUncertainty: dataset
using LinearAlgebra
using MixedModels
using MixedModelsMakie
using PRIMA: bobyqa, issuccess, newuoa
using StatsAPI
...
We will store the response vector, $\bby$, the linear predictor, $\bbeta$, the vector of means, $\bbmu$, and the unit deviances as columns in a table.
Because all the columns are vectors of the same type, `Float64`, we use a `MatrixTable`, which consists of a `Matrix` and a vector of column names, stored as `Symbol`s.
As with other Table types, columns are accessed as properties and often within functions we use "destructuring"
to access them by name.

The `updatetbl!` method iterates over the rows updating the $\bbmu$ and `dev` entries from the `y` and $\bbeta$ values in that row.
...
function updatetbl!(
  ...
  (; y, η, μ, dev) = tbl   # tbl must have columns with these names
...
We illustrate some of these computations using only the fixed-effects specification for `com05`, a GLMM fit to the `contra` data set in @sec-glmmbinomial.
Because we will use the full GLMM later we reproduce `com05` by loading the data, creating the binary `ch` variable indicating children/no-children, defining the contrasts and formula to be used, and fitting the model as in @sec-glmmbinomial.
...
Retain a copy of the estimated fixed-effects coefficient vector, $\bbbeta$.

```{julia}
βm05 = copy(com05.β)
```

Now create a `MatrixTable` with the four columns described above and copy `y` to the first column.
Because we will repeat this operation several times below, we create two methods for a utility function that creates such a table and initializes it to zeros.

```{julia}
#| output: false
function mktbl(
  typ::DataType,
  header::AbstractVector{Symbol},
  n::Integer,
)
  return table(zeros(typ, n, length(header)); header)
end
function mktbl(header::AbstractVector{Symbol}, n::Integer)
  return mktbl(Float64, header, n)
end
```

Create the table, install `y`, evaluate $\bbeta$ in-place from the model matrix, `X`, and the fixed-effects coefficient vector, $\bbbeta$, update the last two columns and display it by converting it to a `TypedTables.Table`.

```{julia}
tbl = mktbl([:y, :η, :μ, :dev], length(com05.y))
copyto!(tbl.y, com05.y)
mul!(tbl.η, com05.X, βm05)  # in-place multiplication to evaluate η
updatetbl!(tbl)
Table(tbl)
```

The deviance for this value of ${\bbbeta}$ in this model is the sum of the unit deviances.

```{julia}
sum(tbl.dev)
```

Benchmarking the operations of updating the table and evaluating the deviance shows they are very fast.

```{julia}
#| label: benchmarkupdatetbl
@b tbl sum(updatetbl!(_).dev)
```
...
In addition to `X`, `β`, and the table, `tbl` (which contains `y`), we include a "reference" to an integer, `neval`, which will be used to keep track of the number of function evaluations to convergence.

:::{.callout-note collapse="true"}

### What's a "reference"

A `RefValue{Int}` is a reference to a memory location that holds a single `Int`.
It is similar to creating a `Vector{Int}` of length 1 without the overhead of the `Vector` type.
The reason for having the `neval` field be a reference to an `Int` rather than an `Int` is so that the value can be modified.
:::
...
struct BernoulliGLM{T<:AbstractFloat}
  ...
  β::Vector{T}
  tbl::MatrixTable{Matrix{T}}
  neval::Base.RefValue{Int} # number of deviance evaluations to optimize
end
```
...
function BernoulliGLM(
  ...
  # initial β from linear regression on X of y in {-1,1} coding
  β = X \ replace(y, 0 => -1)

  tbl = mktbl([:y, :η, :μ, :dev], n)
  copyto!(tbl.y, y)
  mul!(tbl.η, X, β)            # evaluate η from X and β
  return BernoulliGLM(X, β, updatetbl!(tbl), Ref(-1))
end
```
...
```{julia}
StatsAPI.deviance(m::BernoulliGLM) = sum(m.tbl.dev)
```
...
We also define a mutating function, `setβ!`, that installs a new value of `β` then updates `tbl`.

```{julia}
#| output: false
function setβ!(m::BernoulliGLM, newβ)
  mul!(m.tbl.η, m.X, copyto!(m.β, newβ)) # η = X * newβ in place
  updatetbl!(m.tbl)
  return m
end
```
...
We can use a general optimizer like those available in the [PRIMA.jl](https://github.com/libprima/PRIMA.jl) package to minimize the deviance.
In this case we use the `newuoa` optimizer [@powell2006newuoa] for unconstrained problems.

```{julia}
#| output: false
function StatsAPI.fit!(m::BernoulliGLM{T}) where {T}
  obj(x) = deviance(setβ!(m, x))  # "one-liner" def of objective func
  x, info = newuoa(obj, m.β; scale=abs.(m.β))
  issuccess(info) || error("newuoa returned $(info.status)")
  m.neval[] = info.nf
  return setβ!(m, x)
end
```
...
```{julia}
deviance(fit!(com05fe))
```

The optimizer has determined a coefficient vector that reduces the deviance to `{julia} repr(deviance(com05fe); context=:compact => true)`, at which point convergence was declared.
This required `{julia} com05fe.neval[]` evaluations of the deviance at candidate values of $\bbbeta$.

Each evaluation of the deviance is fast, requiring only a fraction of a millisecond

```{julia}
@b (com05fe, com05fe.β) setβ!(first(_), last(_))
```

but the already large number of evaluations for these six coefficients would not scale well as this dimension increases.

The total time to fit the model is

```{julia}
@b BernoulliGLM(com05.X, com05.y) fit!
```
...
We define a `BernoulliIRLS` struct with three additional elements in the `tbl` field: the square roots of the working weights, `rtwwt`, the weighted working residuals, `wwres`, and the weighted working response, `wwresp`.
In the discussion above, `rtwwt` is the diagonal of $\bbW^{1/2}$, `wwres` is $\bbW^{1/2}\tilde{\bbr}$ and `wwresp` is $\bbW^{1/2}\tilde{\bby}$.

We also add fields `Xqr`, in which the weighted model matrix, $\bbW^{1/2}\bbX$, is formed (and subsequently its QR decomposition), `βcp`, which holds a copy of the previous coefficient vector, and `Whalf` which is the diagonal matrix $\bbW^{1/2}$.

```{julia}
#| output: false
struct BernoulliIRLS{T<:AbstractFloat}
  X::Matrix{T}
  Xqr::Matrix{T}              # copy of X to be used for the QR decomp
  β::Vector{T}
  βcp::Vector{T}              # copy of previous β
  Whalf::Diagonal{T}          # rtwwt as a Diagonal matrix
  tbl::MatrixTable{Matrix{T}}
  neval::Base.RefValue{Int}
end
```
...
function BernoulliIRLS(
  ...
  if length(offset) ∉ (0, n)
    throw(
      ArgumentError(
        "length(offset) = $(length(offset)) should be 0 or $n",
      ),
    )
  end

  tbl =                       # create and populate the table
    mktbl([:y, :offset, :η, :μ, :dev, :rtwwt, :wwres, :wwresp], n)
  copyto!(tbl.y, y)
  copyto!(tbl.offset, offset) # if offset is empty this has no effect

  # initial β from linear least squares fit of y in {-1,1} coding
  Xqr = copy(X)
  β = qr!(Xqr) \ replace(y, 0 => -1)
  mul!(tbl.η, X, β)           # create η by in-place multiplication 

  Whalf = Diagonal(tbl.rtwwt) # diagonal matrix of tbl.rtwwt
  return updatetbl!(
    BernoulliIRLS(X, Xqr, β, copy(β), Whalf, tbl, Ref(-1)),
  )
end
```

The named argument `offset` can specify an offset vector.
It defaults to an empty vector, in which case the `offset` column of `tbl` is left in its initial state, which is a vector of zeros.

The `updatetbl!` method for a `BernoulliIRLS` object evaluates the mean vector, the unit deviances, the square roots of the weights, the weighted working residuals, and weighted working responses from the `y`, `offset`, and $\eta$ columns.
The `offset` column, which is initialized to zero by default, will be used in @sec-PIRLS when we discuss the PIRLS algorithm.

```{julia}
#| output: false
function updatetbl!(m::BernoulliIRLS{T}) where {T<:AbstractFloat}
  (; y, offset, η, μ, dev, rtwwt, wwres, wwresp) = m.tbl
  @inbounds for i in eachindex(y)
    ...
    rtwwt[i] = wt = rtexpmη / denom    # square root of working wt
    wwres[i] = res = (yi - μi) / wt    # weighted working resid
    wwresp[i] = res + wt * (ηi - offset[i])
  end
  return m
end
```
...
```{julia}
#| output: false
StatsAPI.deviance(m::BernoulliIRLS) = sum(m.tbl.dev)
```
...
function updateβ!(m::BernoulliIRLS)
  (; X, Xqr, β, βcp, Whalf, tbl) = m  # destructure m & tbl
  (; y, η, wwresp) = tbl
  copyto!(βcp, β)                     # keep a copy of β
  mul!(Xqr, Whalf, X)                 # weighted model matrix into Xqr
  ldiv!(β, qr!(Xqr), wwresp)          # weighted least squares
  mul!(η, X, β)                       # update η
  return updatetbl!(m)
end
```
...
```{julia}
com05irls = BernoulliIRLS(com05.X, com05.y)
deviance(com05irls)
```
...
```{julia}
#| output: false
function StatsAPI.fit!(m::BernoulliIRLS, β₀=m.β; verbose::Bool=false)
  (; X, β, βcp, tbl, neval) = m
  (; y, η) = tbl
  mul!(η, X, copyto!(β, β₀))   # install the starting value
  updatetbl!(m)
  olddev = deviance(m)
  ...
  for i in 1:100               # perform at most 100 iterations
    ...
    elseif (olddev - newdev) < (1.0e-10 * abs(olddev))
      neval[] = i
      return m                 # exit loop if deviance is stable
    else
      olddev = newdev
    end
  end
...
```{julia}
fit!(com05irls, β₀; verbose=true);
```

The IRLS algorithm has converged in `{julia} com05irls.neval[]` iterations to essentially the same deviance as the general optimizer achieved after `{julia} com05fe.neval[]` function evaluations.
...
```{julia}
@b com05irls updateβ!  # benchmark the update
```

The total time to fit the model using IRLS is

```{julia}
@b fit!(BernoulliIRLS(com05.X, com05.y))
```

:::{.callout-note collapse="true"}

### Alternative version of the IRLS algorithm

The preceeding text presented a fixed-point version of the IRLS algorithm.
There is another version of the IRLS algorithm which expresses the next iteration step for $\beta$ as a Newton-Raphson update with the following update equation:
$$
  \bbbeta^{(k)}\ =\ \bbbeta^{(k-1)} + \alpha \times \left(\bbX'\bbW\bbX\right)^{-1} \bbX'\bbW\tilde{\bbr} ,
$$
where $\alpha$ is a step size parameter.

For $\alpha = 1$, it is easy to see that the Newton-Raphson step is equivalent to the fixed-point formulation
$$
  \bbbeta^{(k)} = \left(\bbX'\bbW\bbX\right)^{-1} \bbX'\bbW\tilde{\bby}
$$
by using the identity $\bbbeta^{(k-1)} = \left(\bbX'\bbW\bbX\right)^{-1} \left(\bbX'\bbW\bbX\right) \bbbeta$ and combing the two terms on the right hand side.
For more details see [this RPubs tutorial](https://rpubs.com/enwuliu/1011024) by Enwu Liu.

The Newton-Raphson formulation is used by [GLM.jl](https://juliastats.org/GLM.jl/stable/), but is less suitable for penalized IRLS (PIRLS), which we use below to extend this approach to generalized linear mixed models.
...
struct BernoulliPIRLS{T<:AbstractFloat,S<:Integer}
  irls::BernoulliIRLS{T}
  θβ::Vector{T}
  refs::Vector{S}
  utbl::MatrixTable{Matrix{T}}  # table of vectors like u
  neval::Base.RefValue{Int}
end
```
...
function BernoulliPIRLS(
  ...
  β = irls.β
  θβ = append!(ones(T, 1), β)   # initial value of θ is 1

  # check that refs contains all values from 1 to maximum(refs) and only these
  refvals = sort!(unique(refs))
  q = length(refvals)
  if refvals ≠ 1:q
    throw(ArgumentError("sort!(unique(refs)) must be 1:$q"))
  end
  length(refs) == size(irls.X, 1) ||
    throw(ArgumentError("length of refs doesn't match irls"))

  ytbl = irls.tbl
  mul!(ytbl.offset, irls.X, β)  # PIRLS offset is from fixed-effects
  utbl = mktbl(T, [:u, :u0, :Ldiag, :pdev, :pdev0, :aGHQ], q)
  return updatetbl!(BernoulliPIRLS(irls, θβ, refs, utbl, Ref(-1)))
end
```
...
The reason for storing both $\bbtheta$ and $\bbbeta$ in a single vector is to provide for their simultaneous optimization with an optimizer such as those in [PRIMA.jl](https://github.com/libprima/PRIMA.jl).
:::

or, to construct from `X`, `y` and `refs`
...
function BernoulliPIRLS(
  ...
  return BernoulliPIRLS(fit!(BernoulliIRLS(X, y)), refs)
end
```

The `utbl` field in a `BernoulliPIRLS` struct contains vectors named `pdev`, `pdev0`, and `aGHQ`, in addition to `u`, `u0`, and `Ldiag`.
These are not used in the PIRLS algorithm or for optimizing Laplace's approximation to the objective.
However, they will be used in adaptive Gauss-Hermite quadrature evaluation of the objective (@sec-aGHQ), so we incorporate them in this table throughout.

The `updatetbl!` method for this type first evaluates $\bbeta$ via a "virtual" multiplication that forms $\bbZ\bbLambda_{\bbtheta}\bbu$ plus the stored `offset`, which is $\bbX\bbbeta$, then updates `m.irls.tbl`.
...
function updatetbl!(m::BernoulliPIRLS)
  ...
  if !iszero(θ)
    u = m.utbl.u
    refs = m.refs
    @inbounds for i in eachindex(η, refs)
      η[i] += u[refs[i]] * θ
    end
  end
  updatetbl!(m.irls)
...
function pdeviance(m::BernoulliPIRLS)
  return sum(m.irls.tbl.dev) + sum(abs2, m.utbl.u)
end
```
...
function updateu!(m::BernoulliPIRLS)
  ...
  if iszero(θ)                      # skip the update if θ == 0
    ...
    @inbounds for (i, ri) in enumerate(m.refs)
      rtWΛ = θ * tbl.rtwwt[i]       # non-zero in i'th row of √WZΛ
      Ldiag[ri] += abs2(rtWΛ)       # accumulate Λ'Z'WZΛ
      u[ri] += rtWΛ * tbl.wwresp[i] # accumulate Λ'Z'Wỹ
    end
    Ldiag .+= 1                     # form diagonal of Λ'Z'WZΛ + I = LL'
    u ./= Ldiag                     # solve for u with diagonal LL'
  end
  return updatetbl!(m)              # and update m.irls.tbl
end
```
...
```{julia}
m = BernoulliPIRLS(com05irls, only(com05.reterms).refs)
pdeviance(m)
```
...
function pirls!(m::BernoulliPIRLS; verbose::Bool=false)
  ...
  for i in 1:50                # maximum of 50 PIRLS iterations
    ...
    if newpdev > oldpdev       # PIRLS iteration failed
      ...
      updatetbl!(m)            # restore η and m.irls.tbl
      break
    elseif (oldpdev - newpdev) < (1.0e-8 * abs(oldpdev))
      copyto!(u0, u)           # keep a copy of u
      break
    else
      copyto!(u0, u)           # keep a copy of u
  ...
  Ldiag .= sqrt.(Ldiag)        # replace diag(LL') by diag(L)
  return m
end
```
...
```{julia}
@b BernoulliPIRLS(com05.X, com05.y, only(com05.reterms).refs) pirls!
```
...
The remaining step is to optimize Laplace's approximation to the GLMM deviance with respect to $\theta$ and $\bbbeta$, which we do using the BOBYQA optimizer from [PRIMA.jl](https://github.com/libprima/PRIMA.jl)
...
function StatsAPI.fit!(m::BernoulliPIRLS)
  ...
  function obj(x)
    copyto!(θβ, x)
    mul!(m.irls.tbl.offset, m.irls.X, mβ)
    return laplaceapprox(pirls!(m))
  end
  xl = fill!(similar(θβ), -Inf)   # vector of lower bounds
  xl[1] = 0                       # scalar θ must be non-negative
  x, info = bobyqa(obj, θβ; xl, scale=abs.(θβ))
  issuccess(info) || error("bobyqa returned code $(info.status)")
  m.neval[] = info.nf
  copyto!(θβ, x)
  return m
end
...
This converged at θ = `{julia} repr(first(m.θβ), context=:compact => true)` and β =
`{julia} repr(view(m.θβ, 2:length(m.θβ)), context=:compact => true)` after `{julia} m.neval[]` evaluations of the objective.

These estimates differ somewhat from those for model `com05` of fmin = `{julia} repr(deviance(com05), context=:compact => true)` at θ = `{julia} repr(only(com05.θ), context=:compact => true)` and β = `{julia} repr(com05.β, context=:compact => true)`.
...
There is an implicit assumption in the `BernoulliPIRLS` structure that random effects in the model are simple, scalar random effects associated with a single grouping factor, which is represented by `m.refs`.
...
function pdevcomps!(m::BernoulliPIRLS)
  ...
  @inbounds for (ri, di) in zip(m.refs, m.irls.tbl.dev)
    pdev[ri] += di
  end
  return m
end
...
We begin with plots of the difference in the penalized deviance from its value at the conditional mode, $\tilde{d}_j(u_j)-\tilde{d_j}(\tilde{u}_j)$, for $j=1,\dots,5$ (@fig-uscalepdev).
...
then shift and scale the horizontal axis to the $z$ scale for this difference in the penalized deviance (from that at the conditional mode) (@fig-zscalepdev).
...
The next stage is to plot, on the $z$ scale, the difference between the penalized deviance and its quadratic approximation at $z=0$ (@fig-zpdevdiff)
...
and, finally, the exponential of negative half of this difference (@fig-zexpneghalfdiff)
...
function evalGHQ!(m::BernoulliPIRLS; nGHQ::Integer=9)
  (; u, u0, Ldiag, pdev, pdev0, aGHQ) = m.utbl
  ghqtbl = GHnorm(nGHQ)
...
This converged to θ = `{julia} repr(first(m.θβ), context=:compact => true)` and β =
`{julia} repr(view(m.θβ, 2:length(m.θβ)), context=:compact => true)`.
