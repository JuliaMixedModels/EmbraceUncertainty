...
com05 = let d = contra,
    ds = Bernoulli(),
    f = @formula(use ~ 1 + urban + ch * age + age & age + (1 | dist & urban))
...
```{julia}
#| output: false
function BernoulliGLM(X::Matrix{T},
...
function StatsAPI.fit!(m::BernoulliGLM{T}) where {T}
    ...
    function objective(x::Vector{T}, g::Vector{T}) where {T}
        isempty(g) || throw(ArgumentError("Gradient not available, g must be empty"))
...
```{julia}
βopt = copy(com05fe.β)
@benchmark deviance(setβ!(m, β)) seconds = 1 setup = (m = com05fe; β = βopt)
```
...
struct BernoulliIRLS{T<:AbstractFloat}
    ...
    ytbl::NamedTuple{(:y, :η),NTuple{2,Vector{T}}}
    rtbl::Vector{NamedTuple{(:μ, :dev, :rtwwt, :wwres, :wwresp),NTuple{5,T}}}
end
```
...
```{julia}
#| output: false
function BernoulliIRLS(X::Matrix{T},
                       y::Vector{T}) where {T<:AbstractFloat}
...
function tblrow(y::T,
                η::T,
                offset::T=zero(T)) where {T<:AbstractFloat}
...
struct BernoulliPIRLS{T<:AbstractFloat,S<:Integer}
    X::Matrix{T}
    θβ::Vector{T}
    ytbl::NamedTuple{(:refs, :y, :η, :offset),
                     Tuple{Vector{S},Vector{T},Vector{T},Vector{T}}}
    utbl::NamedTuple{(:u, :u0, :Ldiag, :pdev, :pdev0, :aGHQ),
                     NTuple{6,Vector{T}}}
    rtbl::Vector{NamedTuple{(:μ, :dev, :rtwwt, :wwres, :wwresp),NTuple{5,T}}}
end
```
...
```{julia}
#| output: false
function BernoulliPIRLS(X::Matrix{T},
                        y::Vector{T},
                        refs::Vector{S}) where {T<:AbstractFloat,S<:Integer}
    ...
    utbl = NamedTuple(nm => zeros(T, q) for
...
#| label: fig-ghninelog
#| warning: false
draw(data(gausshermitenorm(9)) * mapping(:abscissae => "Positions",
...
#| output: false
#| code-fold: show
function pdevdiff(m::BernoulliPIRLS{T};
