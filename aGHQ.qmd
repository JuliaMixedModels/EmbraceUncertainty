...
We can use a general optimizer like those available in [PRIMA.jl](https://github.com/libprima/PRIMA.jl) to minimize the deviance.
...
function StatsAPI.fit!(m::BernoulliGLM{T}) where {T}
  obj(x) = deviance(setβ!(m, x))  # objective function
  x, info = newuoa(obj, m.β; scale=abs.(m.β))
  issuccess(info) || throw(ArgumentError("newuoa returned $(info.status)"))
...
This required about 150 evaluations of the deviance at candidate values of $\bbbeta$.
...
The IRLS algorithm has converged in 4 iterations to essentially the same deviance as the general optimizer achieved after more than 800 function evaluations.
