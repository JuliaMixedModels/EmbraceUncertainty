...
using Chairmarks: @b, @be # like @benchmark but better "back support"
...
Now create a `MatrixTable` with the four columns described above

```{julia}
tbl = table(
  reshape(repeat(com05.y, 4), :, 4);
  header=(:y, :η, :μ, :dev),
)
typeof(tbl)
```

and evaluate $\bbeta$ in-place from the model matrix, `X`, and the fixed-effects coefficient vector, $\bbbeta$
...
For display we convert the MatrixTable to a `TypedTables.Table`.
...
function BernoulliGLM(
  ...
  # initial β from linear regression of y in {-1,1} coding on X
  β = X \ replace(y, 0 => -1)

  header = (:y, :η, :μ, :dev)  # create a MatrixTable with these columns
  tbl = table(Matrix{T}(undef, n, length(header)); header)
  copyto!(tbl.y, y)            # copy y into table
  mul!(tbl.η, X, β)            # evaluate η from X and β
  return BernoulliGLM(X, β, updatetbl!(tbl))
end
```
...
In this case we use the `newuoa` optimizer [@powell2006newuoa], provided by `libprima` for unconstrained problems.

```{julia}
function StatsAPI.fit!(m::BernoulliGLM{T}) where {T}
  obj(x) = deviance(setβ!(m, x))  # "one-liner" def of objective function
  x, info = newuoa(obj, m.β; scale=abs.(m.β))
  issuccess(info) ||
    throw(ArgumentError("newuoa returned $(info.status)"))
...
The optimizer has determined a coefficient vector that reduces the deviance to 2409.38, at which point convergence was declared.
This required over 150 evaluations of the deviance at candidate values of $\bbbeta$.

Each evaluation of the deviance is fast, requiring only a fraction of a millisecond on a laptop computer, as shown in this benchmark

```{julia}
@be (com05fe, copy(com05fe.β)) deviance(setβ!(first(_), last(_)))
```
...
We also add fields `Xqr`, in which the weighted model matrix, $\bbW^{1/2}\bbX$, is formed and decomposed to a QR decomposition, `βcp`, which holds a copy of the previous coefficient vector, and `Whalf` which is a diagonal matrix for which the diagonal is `tbl.rtwwt`.
...
function BernoulliIRLS(
  ...
  header = (:y, :offset, :η, :μ, :dev, :rtwwt, :wwres, :wwresp)
  tbl = table(Matrix{T}(undef, length(y), length(header)); header)
  copyto!(tbl.y, y)
  mul!(tbl.η, X, β)
  if isempty(offset)
    fill!(tbl.offset, zero(T))
  else
    copyto!(tbl.offset, offset)
  end
  return updatetbl!(
    BernoulliIRLS(X, Xqr, β, copy(β), Diagonal(tbl.rtwwt), tbl),
  )
end
```

The `updatetbl!` method for a `BernoulliIRLS` object evaluates the mean, unit deviance, square root of the weight, and the weighted, working residual and weighted, working response from the `y`, `offset`, and $\eta$ columns.
...
function updatetbl!(m::BernoulliIRLS{T}) where {T<:AbstractFloat}
  ...
  @inbounds for i in eachindex(y)
    ...
    rtwwt[i] = wt = rtexpmη / denom # sqrt of working wt
...
function updateβ!(m::BernoulliIRLS)
  ...
  copyto!(η, wwresp)                  # use η as temp storage
...
```{julia}
@b com05fe deviance(updateβ!(_))  # benchmark the update
```
...
function BernoulliPIRLS(
  ...
  β = irls.β
  θβ = append!(ones(T, 1), β)  # initially set θ to 1
  ...
  header = (:u, :u0, :Ldiag, :pdev, :pdev0, :aGHQ)
  utbl = table(zeros(T, q, length(header)); header)
  return updatetbl!(BernoulliPIRLS(irls, θβ, refs, utbl))
end
```
...
```{julia}
m = BernoulliPIRLS(com05fe, only(com05.reterms).refs)
pdeviance(m)
```
...
```{julia}
@b BernoulliPIRLS(com05fe, com05re.refs) pirls!(_; verbose=false)
```
