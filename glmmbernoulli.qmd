...
com01 = let d = contra,
    ds = Bernoulli(),
    f = @formula(use ~ 1 + livch + (age + abs2(age)) * urban + (1 | dist))
...
```{julia}
com03 = let f = @formula(use ~ 1 + urban + ch + age * age + (1 | dist)),
    d = contra,
    ds = Bernoulli()
...
```{julia}
com04 = let f = @formula(use ~ 1 + urban + ch * age + abs2(age) + (1 | dist)),
    d = contra,
    ds = Bernoulli()
...
```{julia}
com05 = let f = @formula(use ~ 1 + urban + ch * age + abs2(age) + (1 | dist & urban)),
    d = contra,
    ds = Bernoulli()
...
```{julia}
newdata = Table((; age, ch, urban)
                for age in -10:3:20, ch in [false, true],
                    urban in ["N", "Y"])
```
...
```{julia}
feform = filter(t -> !isa(t, MixedModels.AbstractReTerm),
                com05.formula.rhs);
```
