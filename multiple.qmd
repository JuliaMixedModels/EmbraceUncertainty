...
let
    ...
    mp = mapping(:diameter => "Diameter of inhibition zone [mm]",
                 :plate => "Plate";
                 color=:sample,)
...
#| warning: false
draw(data(@subset(pnm01pars, :type == "β")) *
...
#| code-fold: true
#| warning: false
draw(data(@subset(pnm01pars, :type == "σ")) *
...
#| code-fold: true
#| warning: false
draw(data(@subset(psm01pars, :type == "σ")) *
     mapping(:value => "Bootstrap samples of σ";
             color=(:group => "Group"),) *
     AlgebraOfGraphics.density();
     figure=(; size=(600, 340)),)
...
```{julia}
#| code-fold: true
psm02samp = parametricbootstrap(Random.seed!(9753579),
                                10_000,
                                psm02;
                                progress,)
...
#| warning: false
draw(data(Table(; response=1:5, count=counts(insteval.y))) *
...
```{julia}
iem03 = let f = @formula y ~ 1 + service + (1 | s) + (1 | d) + (1 | dept)
    fit(MixedModel, f, insteval; contrasts, progress)
end
```
...
#| warning: false
draw(data(combine(groupby(DataFrame(insteval), :d), nrow => :n)) *
...
```{julia}
last(sort(DataFrame(ranefinfotable(ranefinfo(iem01, :d))), :cmode),
     5)
```
