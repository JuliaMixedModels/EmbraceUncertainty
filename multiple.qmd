...
### `lrtest` function for nested models
...
`lrtest` is a generic function designed to work on all types of regression models, while `likelihoodratiotest` is completely specialized for the types used in `MixedModels.jl`. This results in a few differences between the functions:
- `likelihoodratiotest` includes the model formulae in the output, while `lrtest` does not because not all regression models are defined using the formula interface
