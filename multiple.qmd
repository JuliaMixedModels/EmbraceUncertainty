...
@isdefined(progress) || const progress = isinteractive()
```
...
We may be able to impose balance in a designed experiment but we typically cannot expect that data from an observational study will be balanced.
...
We can also use the `lrtest` function defined by the `StatsModels` package as follows:
```{julia}
lrtest(psm02, psm01)
```

`lrtest` is a generic function designed to work on all types of regression models, while `likelihoodratiotest` is completely specialized for the types used in `MixedModels.jl`. The key user-observable difference is that `likelihoodratiotest` includes the model formulae in the output, while `lrtest` does not because not all regression models are defined using the formula interface.

As of MixedModels v5.0, `likelihoodratiotest` is a wrapper around `lrtest` and thus behaves identically in terms of test statistics. In previous versions of MixedModels.jl, `likelihoodratiotest` used a completely separate implementation from `lrtest` and there were two additional differences:

- `lrtest` performs additional safety checks for nesting of models compared to `likelihoodratiotest`. Nesting isn't always trivial to check for mixed effects models, so the tests performed by `isnested` (which `lrtest` calls behind the scenes) err on the more conservative side, i.e. rejecting some nested models as being non nested. `likelihoodratiotest` does not use `isnested`, but instead uses fewer more basic compatibility tests and leaves it up to the user to make sure that their models are nested.
- `likelihoodratiotest` has methods defined for comparing a model without random effects, i.e. a n non mixed model, to mixed models. This comparison is useful when checking whether the sole random effect offers any improvement to fit compared to the classical regression model. (When that random effect is singular, then it per definition does not improve model fit!)
...
Even as a sparse matrix `L` requires a considerable amount of memory, about 17 MB,

```{julia}
(Base.summarysize(L), Base.summarysize(collect(L)))
```

but as a triangular dense matrix it requires nearly 8 times as much (about 135 MB).
...
For example, the `EffectsCoding` and `HelmertCoding` contrasts will both assign -1 to the first level (`N` in this case) and +1 to the second level (`Y`).
