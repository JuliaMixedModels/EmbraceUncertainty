...
For nested models we can also use the `lrtest` function defined by the `StatsModels` package as follows:
```{julia}
lrtest(psm02, psm01)
```
However, unlike `MixedModels.likelihoodratiotest`, the `lrtest` function does not work for non-nested models.
