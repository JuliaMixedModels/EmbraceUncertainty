...
```{julia}
ldttrial =
  transform!(DataFrame(ldttrial), :subj => ByRow(s -> string('S', lpad(s, 3, '0'))) => :subj)
describe(ldttrial)
```
...
```{julia}
ldttrial =
  transform!(ldttrial, :seq => ByRow(>(2000)) => :s2)
describe(ldttrial)
```
...
elpldtsubj =
  transform!(elpldtsubj, :subj => ByRow(s -> string('S', lpad(s, 3, '0'))) => :subj)
