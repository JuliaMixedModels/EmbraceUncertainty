...
```{julia}
ldttrial = DataFrame(dataset(:ELP_ldt_trial))
```
...
```{julia}
ldttrial =
  transform!(ldttrial, :subj => (s -> string.('S', lpad.(s, 3, '0'))) => :subj)
describe(ldttrial)
```
...
::: {.callout-note}
RK: In the next chunk `linear(; interval=:confidence)` throws an error for me. I commented it out.
:::
...
let
  ...
  plt *= (visual(Violin) ) #rk: + linear(; interval=:confidence))
  draw(
    plt,
...
elpldtsubj =
  transform!(elpldtsubj, :subj => (s -> string.('S', lpad.(s, 3, '0'))) => :subj)
