...
```{julia}
ldttrial = transform!(DataFrame(ldttrial),
                      :seq => ByRow(>(2000)) => :s2;)
describe(ldttrial)
```
...
#| warning: false
let wlen = 1:21
    draw(data((; wrdlen=wlen, count=counts(byitem.wrdlen, wlen))) *
...
#| warning: false
draw(data(filter(:acc => !ismissing, ldttrial)) *
     mapping(:wrdlen => "Word length",
             :acc => "Accuracy";
             color=:isword,) *
     smooth();
     figure=(; size=(600, 340)),)
...
#| warning: false
draw(data(@subset(ldttrial, !ismissing(:acc), 4 ≤ :wrdlen ≤ 13)) *
     mapping(:wrdlen => "Word length",
             :acc => "Accuracy";
             color=:isword,) *
     smooth();
...
#| warning: false
let
    itemsummry = combine(groupby(byitem, [:wrdlen, :isword]),
                         :ni => sum,
                         :imiss => sum,
                         :iacc => sum)
    @transform!(itemsummry,
                :iacc_mean = :iacc_sum / (:ni_sum - :imiss_sum))
    @transform!(itemsummry, :msz = sqrt((:ni_sum - :imiss_sum) / 800))
    draw(data(itemsummry) * mapping(:wrdlen => "Word length",
...
bysubj = @chain ldttrial begin
    groupby(:subj)
    @combine(:ns = length(:acc),               # no. of obs
...
#| warning: false
draw(data(bysubj) *
     mapping(:spropacc => "Proportion accurate",
...
#| warning: false
draw(data(pruned) *
     mapping(:rt => (x -> 1000 / x) => "Response speed (s⁻¹) for correct responses") *
     AlgebraOfGraphics.density();
     figure=(; size=(600, 340)),)
```
...
```{julia}
count(r -> !ismissing(r.acc) && 200 < r.rt < 333,
      eachrow(ldttrial)) / count(!ismissing, ldttrial.acc)
```
...
let
    ...
    plt *= mapping(:wrdlen => "Word length",
                   :rt => (x -> 1000 / x) => "Speed of response (s⁻¹)";
                   color=:isword)
...
```{julia}
elm01 = let f = @formula 1000 / rt ~ 1 + isword * wrdlen + (1 | item) + (1 | subj)
...
```{julia}
elm02 = let f = @formula 1000 / rt ~ 1 + isword * wrdlen + (1 | item) + (1 | subj)
...
condmeans = leftjoin!(leftjoin!(rename!(DataFrame(raneftables(elm01)[:item]),
                                        [:item, :elm01]),
                                rename!(DataFrame(raneftables(elm02)[:item]),
                                        [:item, :elm02]);
                                on=:item,),
                      select(byitem, :item, :isword; copycols=false);
                      on=:item,)
draw(data(condmeans) *
     mapping(:elm01 => "Conditional means of item random effects for model elm01",
...
```{julia}
elm01a = restoreoptsum!(let f = @formula 1000 / rt ~ 1 + isword * wrdlen + (1 | item) +
                                                     (1 | subj)
