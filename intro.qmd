...
The important characteristic of a categorical covariate is that, at each observed value of the response, the covariate takes on the value of one of a set of distinct levels.[^multimember]

[^multimember]: There are extensions to the classic mixed-effects model, where an observation may belong to multiple levels at the same time. For example, a publication may be associated with multiple authors. For now, we will consider the simpler and more common case of single membership.
...
By default, this compact display of mixed-effects model fits is used in HTML or LaTeX output with Julia code chunks.
...
The line labeled `batch` in the random effects table shows that the random effects added to the term, one for each level of the factor `batch`, are modeled as random variables whose unconditional variance is estimated as 1388.33 g² in the ML fit.
...
The line labeled `Residual` in this table gives the estimate of the variance of the residuals (also in g²) and its corresponding standard deviation.
