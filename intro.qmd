...
The test statistic, `{julia} repr(abs(only(diff(collect(lrt01.deviance)))); context=:compact=>true)`, which is the difference of the `Deviance` values, is compared to a $\chi^2$ distribution on 1 degree of freedom, producing a p-value of $0.1542$.
...
By taking the signed square root of the LR test statistic, which is approximately distributed as a $\chi^2$ on 1 degree of freedom (i.e. the square of a standard Gaussian, i.e. "normal", distribution) we obtain a test statistic that can be compared to a standard Gaussian distribution.
...
This is not surprising in that there are only 30 observations in the `dyestuff` data and estimators based on a small number of data points tend to be over-dispersed.
...
It turns out that it is difficult to detect over-dispersion in a density plot, which is why we tend to use normal probability plots to detect this in a sample.
...
The confidence intervals on $\sigma_1$ are even more unusual.
The parameter is bounded below at $\sigma_1=0$, which corresponds to a likelihood ratio test p-value of 0.0201
...
Thus for any confidence level greater than 97.99%, these confidence intervals on $\sigma_1$ includes zero.
...
The profile density curve for the parameter $\sigma_1$ goes to zero smoothly as $\sigma_1$ descreases to zero but this is somewhat misleading.
