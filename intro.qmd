...
```{julia}
const progress = false    # used in calls to fit to suppress progress bar
...
```{julia}
dsm01 = fit(MixedModel, @formula(yield ~ 1 + (1|batch)), dyestuff; progress)
...
```{julia}
dsm01trace = fit(MixedModel, @formula(yield ~ 1 + (1|batch)), dyestuff; thin=1, progress)
