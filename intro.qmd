...
The distribution of the estimates of `β₁` is more-or-less a Gaussian (or "normal") shape, with a mean value of `{julia} compact(mean(βdf.value))`  which is close to the estimated `β₁` of `{julia} compact(only(dsm01.β))`.

Similarly the standard deviation of the simulated β values, `{julia} compact(std(βdf.value))` is close to the standard error of the parameter, `{julia} compact(only(stderror(dsm01)))`.
...
There is one peak around the "true" value for the simulation, `{julia} compact(only(dsm01.σs.batch)), and another peak at zero.
...
Here the algorithm converges after 18 function evaluations to a profiled deviance of `{julia} compact(dsm01trace.objective)` at θ = `{julia} compact(only(dsm01trace.theta))`.
...
However, when the number of levels is large, as for some of the models in later chapters, failure to specify `Grouping()` contrasts can cause memory faults when constructing the numeric representation of the model.
