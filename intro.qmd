...
:::{.callout-note}
In this code block the model is "printed" using `println` after being fit.
By default, mixed-effects models in Jupyter notebooks, which is the evaluation engine behind quarto.org books, a more compact presentation
:::

```{julia}
const progress = false    # suppress the progress bar in the fit
dsm01 = fit(
    MixedModel,
...
:::{.callout-note}
In this code block the model is "printed" using `println` after being fit.
By default, mixed-effects models in Jupyter notebooks, which is the evaluation engine behind quarto.org books, are shown more compactly.
:::
...
```{julia}
dsm02 = fit(MixedModel, @formula(yield ~ 1 + (1|batch)), dyestuff2; progress)
println(dsm02)
```
...
bsrng = Random.seed!(4321234) # random number generator for bootstrap
dsm01samp = parametricbootstrap(bsrng, 10_000, dsm01; hide_progress)
...
caterpillar!(Figure(resolution=(800, 250)), ranefinfo(dsm01, :batch))
```
...
qqcaterpillar!(Figure(resolution=(800, 250)), ranefinfo(dsm01, :batch))
```
