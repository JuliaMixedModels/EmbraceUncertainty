...
The optional, named argument, `progress=false`, suppresses the creation and display of a progress meter for the iterations.
Such displays are useful in interactive sessions to monitor the progress of the iterations but not in a static display for a book.
The comma used to separate argument values can be replaced by a semi-colon when it separates positional arguments, which must precede named arguments, from the first named argument.
...
but current versions of the `MixedModels` package do this automatically.
