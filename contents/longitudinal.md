...
Data from a dental study measuring the lengths of the ramus bone (mm) in 20 boys at 8, 8.5, 9, and 9.5 years of age were reported in @elstongrizzle1962 and in @davis2002.

```jl
sco("""elstongrizzle = Arrow.Table("./data/elstongrizzle.arrow")""")
```
...
## A model with random effects for slope and intercept
...
```jl
sco("""
egm02 = fit(MixedModel,
    @formula(resp ~ 1 + time + (1+time|Subj)),
    egdf;
    contrasts=Dict(:time => Center(8))
)""")
```
...
The model with the highest fidelity to the data corresponds to a fixed-effects model with the random effects model matrix, $\mathbf{Z}$, incorporated into the fixed-effects model matrix, $\mathbf{X}$.
...
```jl
egm04 = EU.fitegm04(egdf)
```

We see that the estimate for the population quadratic coefficient, `jl last(egm04.beta)`, is small, relative to its standard error, `jl last(egm04.stderror)`, indicating that it is not significantly different from zero.
...
```jl
sco("MixedModels.likelihoodratiotest(egm03, egm04)")
```
...
sco("describe(bxdf)", process=without_caption_label)
```

There are three treatment groups

```jl
sco("levels(bxdf.Group)")
```

and each "subject" (rat, in this case) is in only one of the treatment groups.
This can be checked by comparing the number of unique `Subj` levels, `jl length(unique(bxdf.Subj))`, to the number of unique combinations of `Subj` and `Group`.

```jl
scob("nrow(unique(select(bxdf, :Group, :Subj)))")
```
...
```jl
sco("combine(groupby(unique(select(bxdf, :Group, :Subj)), :Group), nrow => :n)", process=without_caption_label)
```
...
> **NOTE** Is there a way to write the formula for `bxm02` to avoid this?
...
```jl
bxm03 = EU.fitbxm03(bxdf)
```

```jl
sco("MixedModels.likelihoodratiotest(bxm03, bxm02)")
```
...
```jl
EU.bxm03caterpillar(bxm03)
```

A shrinkage plot, @fig:bxm03shrinkage

```jl
EU.bxm03shrinkage(bxm03)
```
...
```jl
bxm03samp = parametricbootstrap(Xoshiro(8642468), 10_000, bxm03)
bxm03pars = DataFrame(bxm03samp.allpars)
sco("DataFrame(shortestcovint(bxm03samp))", process=without_caption_label)
```
...
```jl
EU.bxm03rhodensatanh(bxm03pars)
```
...
```jl
bxm04 = EU.fitbxm04(bxdf)
```

```jl
sco("MixedModels.likelihoodratiotest(bxm04, bxm03)")
```
...
```jl
bxm05 = EU.fitbxm05(bxdf)
```

```jl
sco("MixedModels.likelihoodratiotest(bxm05, bxm03)")
```
...
```jl
EU.bxmodfitted(bxm03.beta, bxm04.beta, bxm05.beta)
```

> **NOTE** Other points to make:
