...
begin
	egplt = data(egdf) * mapping(:time => "Age [yr]", :resp => "Ramus bone length")
	draw(egplt * mapping(color=:Subj) * (visual(Scatter) + visual(Lines)))
end		

# ╔═╡ bacc2adf-d428-4f62-b36e-5e4d3d7500e8
let
	egdf.subj = CategoricalArray(
		egdf.Subj; 
		levels = @sort(@subset(egdf, :time == 8.0), :resp).Subj,
	)
    egplt2 = egplt * mapping(layout=:subj) * visual(ScatterLines)
    draw(egplt2; axis=(height=300, width=120))
end

# ╔═╡ b93d503c-cc11-445d-a83d-9ef18f7d1390
m1 = let
        contr = Dict(:Subj => Grouping())
        form = @formula(resp ~ 1 + time + (1+time|Subj))
        fit(MixedModel, form, egdf; contrasts=contr)
end
...
# ╔═╡ b0f651a2-7574-40b5-b6ef-8c501be9b4fb
m2 = let
        egdf.agec = egdf.time .- 8.0
        contr = Dict(:Subj => Grouping())
        form = @formula(resp ~ 1 + agec + (1+agec|Subj))
        fit(MixedModel, form, egdf; contrasts=contr)
end

# ╔═╡ afab5e2c-ef19-4bdf-b50f-73ab93638d46
VarCorr(m2)

# ╔═╡ a4537a4b-50f0-4d6f-8d3e-dfc6b13b8e55
caterpillar(m2)

# ╔═╡ 67795329-ad64-49fc-899d-8d6ffe109616
m3 = let
        contr = Dict(:Subj => Grouping())
        form = @formula(resp ~ 1 + agec + agec^2 + (1+agec+agec^2|Subj))
        fit(MixedModel, form, egdf; contrasts=contr)
end

# ╔═╡ e9847f91-d95c-4693-b1f8-1feaa0c28e91
VarCorr(m3)

# ╔═╡ c00aaac3-ece1-4e4d-b29b-a4cfe5dd5447
caterpillar(m3)
...
# ╔═╡ 1f7cc71f-5c89-47c6-9b27-4d2289f26f03
objective.([m2, m3])

# ╔═╡ 8c1ec5bc-0613-4231-8888-1465bb41af84
shrinkageplot(m2)

# ╔═╡ b86b177b-b799-4f99-80b1-8dc3ee582cc1
shrinkageplot(m3)
