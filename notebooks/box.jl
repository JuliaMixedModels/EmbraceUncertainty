...
# ╔═╡ ffccd42e-32a6-11ec-3b34-6b57623ee591
begin
...
# ╔═╡ 827c44e4-9c78-4e73-b93f-55260954d3fa
boxtbl = Arrow.Table("../data/box.arrow")

# ╔═╡ 4c3cf5b6-65e8-494e-877e-bba4a3d0a89d
begin
	boxdf = DataFrame(boxtbl)
	describe(boxdf)
end

# ╔═╡ 5218ac26-7d3a-4be7-a44a-5be8282394a0
begin
	boxplt = data(@subset(boxdf, :Group == "Control")) *
		mapping(:time => "Time in trial [wk]", :resp => "Body weight [g]")
	draw(boxplt * mapping(color=:Subj) * (visual(Scatter) + visual(Lines)))
end		

# ╔═╡ 8fc6670b-21f2-4bb9-a64e-a6496887b90f
draw(boxplt * mapping(layout=:Subj) * visual(ScatterLines),
	axis=(height=250, width=150))

# ╔═╡ c1847b24-4a64-4783-9384-2986e3ea30b7
begin
	thyroxin = data(@subset(boxdf, :Group == "Thyroxin")) *
		mapping(
		:time => "Time in trial [wk]",
		:resp => "Body weight [g]";
		layout=:Subj,
		) * visual(ScatterLines)
	draw(thyroxin, axis=(height=250,width=150))
end		

# ╔═╡ a2a87b3b-6986-4473-add7-bfb115463a0e
begin
	thioracil = data(@subset(boxdf, :Group == "Thioracil")) *
		mapping(
		:time => "Time in trial [wk]",
		:resp => "Body weight [g]";
		layout=:Subj,
		) * visual(ScatterLines)
	draw(thioracil, axis=(height=250,width=150))
end

# ╔═╡ 9f8fd8b0-be5e-479e-9383-8c3f819968d0
data(@subset(boxdf, iszero(:time))) * # need to jitter the y-positions and use circles
mapping(:resp => "Weight [g] at start of trial", :Group => "") |> 
draw
...
md"""
The main effects for `Group`, which would correspond to systematic shifts at `time = 0`, are not signficant.
This is not unexpected.
The population is assumed to be homogeneous before the treatment starts.
"""
...
git-tree-sha1 = "2ce5421abf80bfa57b7c9c3cf3e2a62831b03869"
