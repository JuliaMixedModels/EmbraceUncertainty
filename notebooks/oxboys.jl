...
# ╔═╡ 5617d0c8-cd25-11eb-0ad1-635e8c432c9b
begin
...
# ╔═╡ bf817866-11b5-43a3-bc22-e9addb14e470
CairoMakie.activate!(type="svg")

# ╔═╡ 1d76e259-fee6-462c-a7bc-01c2abb81765
begin
  oxboys = DataFrame(Arrow.Table("../data/oxboys.arrow"))
  oxboys.Subjord = CategoricalArray(
    oxboys.Subj; # create an ordered Subj variable
    levels=string.(
      @chain oxboys begin
        groupby(:Subj)
...
begin
  plt =
    data(oxboys) *
    mapping(:age => "Scaled Age", :height => "Height [cm]")
  draw(
    plt * mapping(color=:Subj) * (visual(Scatter) + visual(Lines)),
  )
end

# ╔═╡ a45b53d8-7fd4-4a64-834e-d6b295cd315b
draw(
  plt * mapping(layout=:Subjord) * visual(ScatterLines);
  axis=(height=400, width=200),
)
...
# ╔═╡ aae06125-2674-4e26-a529-6c45e919cc7b
VarCorr(m1)

# ╔═╡ 045920bb-02fa-40fa-a95a-3167f2264896
m2 =
  let form =
      @formula(height ~ 1 + age + age^2 + (1 + age + age^2 | Subj))
    fit(MixedModel, form, oxboys)
  end

# ╔═╡ dfc31a6f-2cd3-40fa-b701-e623ce1ea9d4
VarCorr(m2)

# ╔═╡ d01c3ae3-8864-4785-ab4d-c7b33a29562b
only(m2.λ)

# ╔═╡ c98900c9-a07d-4b78-85a5-00c48b941ede
shrinkageplot(m2)

# ╔═╡ 318f53c5-b6cb-451f-b0f8-8e78789958db
caterpillar(m2)

# ╔═╡ ce60b35e-1863-4cf9-bb15-1d02315b0bab
MixedModels.likelihoodratiotest(m1, m2)

# ╔═╡ 0577d42d-8f65-4982-9d2d-d82fe827d0bb
md"""
The difference in counts of estimated parameters between models `m1` and `m2` is 4.
If instead we evaluate the geometric degrees of freedom, which is the trace of the hat matrix or the sum of the leverage values, the effective change is approximately 17.4 degrees of freedom.
"""

# ╔═╡ 39f28f98-e50f-4816-99a5-48bb3ed4ce8c
geomdof(m::LinearMixedModel) = sum(leverage(m));

# ╔═╡ 85221135-a64c-4a6c-a334-be86861b27eb
geomdof.([m1, m2])

# ╔═╡ 346e594a-e5fc-42b8-9dfe-c3f7bc0b0442
only(diff(geomdof.([m1, m2])))

# ╔═╡ 3756166d-fd76-4612-9d95-3b2f448ef792
md"The likelihood ratio test will still indicate a preference for the more complex model."
...
manifest_format = "2.0"
project_hash = "615e337755dab7ba8bbf49237792075d4b26fcef"
